#!/usr/bin/env bash
# Reproducible production smoke test for the Java taskboard fixture.
#
# 1. Builds the shaded jar (VERSION marker + target/deploy-test-java.jar).
# 2. Starts the REAL production process: java -jar target/deploy-test-java.jar.
...
WORK="$(mktemp -d /tmp/java-smoke.XXXXXX)"
...
start_server() { # db_path port logfile -> 0 on success
  ...
  PORT="$port" BIND_HOST="127.0.0.1" BUILD_MARKER="smoke-$port" \
    "$JAVA_BIN" -jar "$ROOT/target/deploy-test-java.jar" >"$logfile" 2>&1 &
  echo $! > "$PIDFILE"
  sleep 0.8
}
...
printf '=== Java smoke start: %s ===\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)"

PORT1="$(pick_port)"
echo "phase 1: production server on 127.0.0.1:$PORT1 (java -jar target/deploy-test-java.jar)"
[ -f "$ROOT/target/deploy-test-java.jar" ] && ok "shaded jar exists (target/deploy-test-java.jar)" || bad "jar missing"
...
case "$meta" in
  *'"name":"java"'*) ok "runtime reports java";;
  *) bad "runtime not reported as java";;
esac
...
case "$index" in
  *"Java Taskboard"*) ok "UI served at /";;
...
echo
printf '=== Java smoke summary: %s passed, %s failed ===\n' "$PASS" "$FAIL"
[ "$FAIL" -eq 0 ]